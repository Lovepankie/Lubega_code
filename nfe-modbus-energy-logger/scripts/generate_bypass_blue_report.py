#!/usr/bin/env python3
"""
Generate charts + PDF analysis report for bypass_blue scenario.
...
BYPASS_PATH = os.path.join(BASE_DIR, 'data', 'bypass_blue.csv')
NORMAL_PATH = os.path.join(BASE_DIR, 'data', 'normal_final.csv')
CHART_DIR   = os.path.join(BASE_DIR, 'docs', 'bypass_blue_charts')
OUTPUT_PDF  = os.path.join(BASE_DIR, 'docs', 'bypass_blue_report.pdf')
...
# Remove clock-drift rows (Pi clock reset to Apr 6 during collection — actual start was 9 Apr)
df = df[df['timestamp'] >= '2026-04-09'].reset_index(drop=True)

df['hour']  = df['timestamp'].dt.hour
dfn['hour'] = dfn['timestamp'].dt.hour

# Actual collection window confirmed by user: 9 Apr 07:21 -> 10 Apr ~01:00 EAT
ACTUAL_START = pd.Timestamp('2026-04-09 07:21:00')
ACTUAL_END   = pd.Timestamp('2026-04-10 01:00:00')
...
# ── Key stats ────────────────────────────────────────────────────────────────
v_mean   = df['V_L3'].mean()
v_zero   = (df['V_L3'] < 1).mean() * 100

i1_mean  = df['I_L1'].mean()
i2_mean  = df['I_L2'].mean()
...
# 1. Voltage L3 comparison
fig, axes = plt.subplots(2, 1, figsize=(14, 7), sharex=False)
axes[0].plot(dfn['timestamp'], dfn['V_L3'], color=NORMAL_GREEN, linewidth=0.6, alpha=0.8, label='Normal L3')
axes[0].axhline(180, color='red', linestyle='--', linewidth=0.8, label='180V threshold')
axes[0].set_title('Voltage L3 — Normal Baseline', fontsize=11, fontweight='bold')
...
axes[1].plot(df['timestamp'], df['V_L3'], color=BLUE, linewidth=0.6, alpha=0.85, label='Bypass Blue L3')
axes[1].axhline(180, color='red', linestyle='--', linewidth=0.8)
axes[1].set_title('Voltage L3 — Bypass Blue (anomaly: drops to 0V)', fontsize=11, fontweight='bold', color=DARK_BLUE)
...
# 2. Current L3 comparison
fig, axes = plt.subplots(2, 1, figsize=(14, 7), sharex=False)
axes[0].plot(dfn['timestamp'], dfn['I_L3'], color=NORMAL_GREEN, **STYLE, label='Normal I_L3')
axes[0].set_title('Current L3 — Normal Baseline', fontsize=11, fontweight='bold')
axes[0].set_ylabel('A'); axes[0].legend(fontsize=8); axes[0].grid(alpha=0.3)
axes[0].xaxis.set_major_formatter(mdates.DateFormatter(FMT))

axes[1].plot(df['timestamp'], df['I_L3'], color=BLUE, **STYLE, label='Bypass Blue I_L3')
axes[1].set_title(f'Current L3 — Bypass Blue ({i3_zero_pct:.1f}% readings near zero)', fontsize=11, fontweight='bold', color=DARK_BLUE)
...
ax.plot(df['timestamp'], df['I_L2'], color='#f9a825',    **STYLE, label='I L2')
ax.plot(df['timestamp'], df['I_L3'], color=BLUE,         **STYLE, label='I L3 (bypassed)')
ax.set_title('All Phase Currents — Bypass Blue Scenario', fontsize=13, fontweight='bold')
...
axes[1].plot(df['timestamp'], df['P_total'], color=BLUE, linewidth=0.6, alpha=0.85)
axes[1].fill_between(df['timestamp'], df['P_total'], alpha=0.15, color=BLUE)
axes[1].set_title('Total Power — Bypass Blue', fontsize=11, fontweight='bold', color=DARK_BLUE)
...
axes[1].plot(df['timestamp'], df['PF_total'], color=BLUE, linewidth=0.6, alpha=0.85)
axes[1].set_title('Power Factor — Bypass Blue (mean %.3f)' % pf_mean, fontsize=11, fontweight='bold', color=DARK_BLUE)
...
ax.plot(range(len(df_norm)),  df_norm.values,  color=BLUE,         linewidth=1, label='Bypass Blue')
...
# 7. Current L3 histogram comparison
fig, axes = plt.subplots(1, 2, figsize=(13, 4))
axes[0].hist(dfn['I_L3'], bins=50, color=NORMAL_GREEN, alpha=0.8, edgecolor='white')
axes[0].set_title('I_L3 Distribution — Normal', fontsize=11, fontweight='bold')
axes[0].set_xlabel('A'); axes[0].set_ylabel('Count'); axes[0].grid(alpha=0.3)

axes[1].hist(df['I_L3'], bins=50, color=BLUE, alpha=0.9, edgecolor='white')
axes[1].set_title('I_L3 Distribution — Bypass Blue', fontsize=11, fontweight='bold', color=DARK_BLUE)
...
# 8. Hourly average current L3
fig, ax = plt.subplots(figsize=(13, 4))
h_normal = dfn.groupby('hour')['I_L3'].mean()
h_bypass = df.groupby('hour')['I_L3'].mean()
x = range(24)
w = 0.38
ax.bar([i - w/2 for i in x], [h_normal.get(i, 0) for i in x], width=w, color=NORMAL_GREEN, alpha=0.85, label='Normal')
ax.bar([i + w/2 for i in x], [h_bypass.get(i, 0) for i in x], width=w, color=BLUE,         alpha=0.85, label='Bypass Blue')
ax.set_title('Hourly Average Current L3 — Normal vs Bypass Blue', fontsize=13, fontweight='bold')
ax.set_xlabel('Hour of Day'); ax.set_ylabel('Avg I_L3 (A)'); ax.legend(); ax.grid(axis='y', alpha=0.3)
ax.set_xticks(range(24))
plt.tight_layout()
plt.savefig(os.path.join(CHART_DIR, '08_hourly_L3_compare.png'), dpi=150); plt.close()
print("  [8/8] 08_hourly_L3_compare.png")
...
h1_s = ParagraphStyle('H1', parent=styles['Heading1'], fontSize=15, spaceBefore=14,
                spaceAfter=6, textColor=colors.HexColor('#0d47a1'))
...
story.append(Paragraph("Bypass Blue Phase — Analysis Report", title_s))
story.append(Spacer(1, 0.2*cm))
story.append(HRFlowable(width=CONTENT_W, thickness=2, color=colors.HexColor('#1e88e5')))
story.append(Spacer(1, 0.5*cm))

meta = [
    ["Scenario:",       "bypass_blue  (Phase C / L3 current transformer shunted)"],
...
mt.setStyle(TableStyle([
    ...
    ('ROWBACKGROUNDS', (0,0),(-1,-1), [colors.HexColor('#e3f2fd'), colors.white]),
...
exec_text = (
    ...
    f"under the <b>bypass_blue scenario</b> — Phase C (L3) current transformer shunted with a parallel wire. "
    f"Key anomaly signatures identified: "
    f"Current L3 drops to near-zero in <b>{i3_zero_pct:.1f}%</b> of readings; "
    f"Voltage L3 collapses to 0V in <b>{v_zero:.1f}%</b> of readings; "
...
story.append(HRFlowable(width=CONTENT_W, thickness=0.5, color=colors.HexColor('#1e88e5')))
story.append(Spacer(1, 0.3*cm))
...
story.append(Paragraph(
    ...
    f"During the bypass_blue scenario, Phase L3 voltage collapses to <b>0V in {v_zero:.1f}% of readings</b> "
...
story.append(img)
story.append(Paragraph("Figure 1: Voltage L3 — normal baseline (top) vs bypass blue (bottom). "
    "Note the frequent collapses to 0V in the bypass scenario.", caption_s))
story.append(Paragraph(
    f"<b>Anomaly signature:</b> V_L3 drops to 0V in {v_zero:.1f}% of samples. "
    f"Normal baseline never drops below 118V. This is the strongest single-feature indicator for bypass_blue.", alert_s))
...
story.append(Spacer(1, 0.3*cm))
story.append(Paragraph(
    f"Current L3 (the bypassed phase) averages <b>{i3_mean:.3f}A</b> and is near-zero "
    f"({i3_zero_pct:.1f}% of readings below 0.05A) — confirming that most current is flowing "
    f"through the bypass wire rather than the meter's CT. "
    f"In contrast, the normal baseline shows I_L3 mean of <b>{n_i3:.3f}A</b>. "
    f"Phase L1 and L2 are unaffected and continue showing normal current levels.", body_s))
img2 = Image(os.path.join(CHART_DIR, '02_current_L3_compare.png'), width=CONTENT_W, height=8*cm)
story.append(img2)
story.append(Paragraph("Figure 2: Current L3 — normal (top) vs bypass blue (bottom). "
    f"The bypassed phase reads near-zero in {i3_zero_pct:.1f}% of samples.", caption_s))
...
story.append(img3)
story.append(Paragraph("Figure 3: All three phase currents during bypass_blue. "
    "I_L3 (blue) is suppressed while L1 and L2 behave normally.", caption_s))
story.append(Paragraph(
    f"<b>Anomaly signature:</b> I_L3 mean drops from {n_i3:.3f}A (normal) to {i3_mean:.3f}A (bypass). "
    f"A {(1-i3_mean/max(n_i3,0.001))*100:.0f}% reduction in reported current on the targeted phase.", alert_s))
...
story.append(img4)
story.append(Paragraph("Figure 4: Total active power — normal (top) vs bypass blue (bottom).", caption_s))
...
story.append(img5)
story.append(Paragraph("Figure 5: Power factor — normal (top) vs bypass blue (bottom). "
...
story.append(img6)
story.append(Paragraph("Figure 6: Cumulative energy gained — normal vs bypass blue (both normalised to 0 at start). "
...
story.append(img7)
story.append(Paragraph("Figure 7: I_L3 histogram — normal (left) vs bypass blue (right). "
    f"The bypass distribution is heavily concentrated at 0A ({i3_zero_pct:.1f}% of samples).", caption_s))

img8 = Image(os.path.join(CHART_DIR, '08_hourly_L3_compare.png'), width=CONTENT_W, height=5.5*cm)
story.append(img8)
story.append(Paragraph("Figure 8: Hourly average I_L3 — normal vs bypass blue by hour of day. "
...
story.append(Spacer(1, 0.3*cm))
story.append(Paragraph(
    "The table below quantifies each anomaly feature for the bypass_blue scenario "
    "against the normal baseline, along with the expected detection method.", body_s))

sig_data = [
    ['Feature', 'Normal', 'Bypass Blue', 'Change', 'Model'],
    ['V_L3 mean (V)',
     f'{dfn["V_L3"].mean():.1f}', f'{v_mean:.1f}',
     f'{v_mean - dfn["V_L3"].mean():+.1f}V', 'IF / SVM'],
    ['V_L3 zero-collapse (%)',
     '0.0%', f'{v_zero:.1f}%', f'+{v_zero:.1f}%', 'IF / LSTM'],
    ['I_L3 mean (A)',
     f'{n_i3:.3f}', f'{i3_mean:.3f}',
     f'{(i3_mean/max(n_i3,0.001)-1)*100:+.0f}%', 'IF / SVM / LSTM'],
    ['I_L3 near-zero (%)',
     '~5%', f'{i3_zero_pct:.1f}%', f'+{i3_zero_pct-5:.0f}%', 'IF / LSTM'],
    ['PF total mean',
    ...
    ['Current imbalance',
     'Low', 'HIGH (L3 suppressed)', 'Significant', 'IF / SVM'],
]
...
story.append(Paragraph(
    f"<b>Conclusion:</b> The bypass_blue scenario produces 4 strong, consistent anomaly signatures: "
    f"voltage collapse, current suppression, PF shift, and reduced energy rate — all on the targeted Phase L3. "
...
story.append(HRFlowable(width=CONTENT_W, thickness=1, color=colors.HexColor('#1e88e5')))
story.append(Spacer(1, 0.2*cm))
story.append(Paragraph(
    f"Generated by NFE data pipeline  |  {datetime.now().strftime('%d %B %Y, %H:%M')}  |  "
    f"Dataset: bypass_blue.csv ({len(df):,} rows)  |  Meter: CHINT DTSU666  |  Kampala, Uganda",
    caption_s))
