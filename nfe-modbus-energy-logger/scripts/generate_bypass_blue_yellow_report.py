#!/usr/bin/env python3
"""
Generate charts + PDF analysis report for bypass_blue_yellow scenario.
Compares against normal_final.csv baseline.
Both L2 (Yellow) and L3 (Blue) are bypassed; L1 (Red) carries load normally.
"""
import os
import pandas as pd
import numpy as np
...
BYPASS_PATH = os.path.join(BASE_DIR, 'data', 'bypass_blue_yellow.csv')
NORMAL_PATH = os.path.join(BASE_DIR, 'data', 'normal_final.csv')
CHART_DIR   = os.path.join(BASE_DIR, 'docs', 'bypass_blue_yellow_charts')
OUTPUT_PDF  = os.path.join(BASE_DIR, 'docs', 'bypass_blue_yellow_report.pdf')
...
# Remove clock-drift rows (6049 rows remain after this filter)
df = df[df['timestamp'] >= '2026-04-11'].reset_index(drop=True)
...
axes[1].set_title(f'All Phase Currents — Bypass Blue+Yellow  |  I_L2={i2_zero_pct:.1f}% near-zero  |  I_L3={i3_zero_pct:.1f}% near-zero',
...
plt.tight_layout()
plt.savefig(os.path.join(CHART_DIR, '01_all_currents.png'), dpi=150); plt.close()
print("  [1/8] 01_all_currents.png")

# 2. I_L2 (Yellow — bypassed) comparison
...
axes[1].plot(df['timestamp'], df['I_L2'], color=YELLOW, **STYLE, label='bypass_blue_yellow — I_L2')
axes[1].set_title(f'Current L2 — Bypass Blue+Yellow  ({i2_zero_pct:.1f}% readings near zero)',
                  fontsize=11, fontweight='bold', color=TEAL)
axes[1].set_ylabel('A'); axes[1].legend(fontsize=8); axes[1].grid(alpha=0.3)
axes[1].xaxis.set_major_formatter(mdates.DateFormatter(FMT))
plt.setp(axes[1].xaxis.get_majorticklabels(), rotation=30)
...
# 3. I_L3 (Blue — bypassed) comparison
fig, axes = plt.subplots(2, 1, figsize=(14, 7), sharex=False)
...
axes[1].plot(df['timestamp'], df['I_L3'], color=BLUE, **STYLE, label='bypass_blue_yellow — I_L3')
axes[1].set_title(f'Current L3 — Bypass Blue+Yellow  ({i3_zero_pct:.1f}% readings near zero)',
                  fontsize=11, fontweight='bold', color=TEAL)
axes[1].set_ylabel('A'); axes[1].legend(fontsize=8); axes[1].grid(alpha=0.3)
axes[1].xaxis.set_major_formatter(mdates.DateFormatter(FMT))
plt.setp(axes[1].xaxis.get_majorticklabels(), rotation=30)
plt.tight_layout()
plt.savefig(os.path.join(CHART_DIR, '03_current_L3_compare.png'), dpi=150); plt.close()
print("  [3/8] 03_current_L3_compare.png")
...
axes[1].plot(df['timestamp'], df['V_L1'], color=RED,    linewidth=0.6, alpha=0.85, label='Bypass BY V_L1')
axes[1].plot(df['timestamp'], df['V_L2'], color=YELLOW, linewidth=0.6, alpha=0.85, label='Bypass BY V_L2')
axes[1].plot(df['timestamp'], df['V_L3'], color=BLUE,   linewidth=0.6, alpha=0.85, label='Bypass BY V_L3')
axes[1].set_title('Voltages — Bypass Blue+Yellow', fontsize=11, fontweight='bold', color=TEAL)
...
axes[1].plot(df['timestamp'], df['P_total'], color=TEAL, linewidth=0.6, alpha=0.85)
axes[1].fill_between(df['timestamp'], df['P_total'], alpha=0.15, color=TEAL)
axes[1].set_title('Total Power — Bypass Blue+Yellow', fontsize=11, fontweight='bold', color=TEAL)
...
axes[1].plot(df['timestamp'], df['PF_total'], color=TEAL, linewidth=0.6, alpha=0.85)
axes[1].set_title('Power Factor — Bypass Blue+Yellow (mean %.3f)' % pf_mean, fontsize=11, fontweight='bold', color=TEAL)
...
ax.plot(range(len(df_norm)),  df_norm.values,  color=TEAL,  linewidth=1, label='Bypass Blue+Yellow')
...
ax.plot(df['timestamp'],  df_imb,  color=TEAL,  linewidth=0.7, alpha=0.85, label='Bypass BY imbalance')
ax.axhline(dfn_imb.mean(), color=GREEN, linestyle='--', linewidth=1.2, label=f'Normal avg imbalance ({dfn_imb.mean():.3f}A)')
ax.set_title('Current Imbalance (Std across L1/L2/L3) — Bypass Blue+Yellow', fontsize=13, fontweight='bold')
...
title_s = ParagraphStyle('T',  parent=styles['Title'],   fontSize=22, spaceAfter=6,
              textColor=colors.HexColor(DARK_TL), alignment=TA_CENTER)
...
h1_s    = ParagraphStyle('H1', parent=styles['Heading1'],fontSize=15, spaceBefore=14,
              spaceAfter=6, textColor=colors.HexColor(DARK_TL))
h2_s    = ParagraphStyle('H2', parent=styles['Heading2'],fontSize=12, spaceBefore=10,
              spaceAfter=4, textColor=colors.HexColor('#00695c'))
...
info_s  = ParagraphStyle('I',  parent=styles['Normal'],  fontSize=10, leading=14,
              backColor=colors.HexColor('#e0f2f1'), spaceBefore=4, spaceAfter=8)
warn_s  = ParagraphStyle('W',  parent=styles['Normal'],  fontSize=10, leading=14,
              backColor=colors.HexColor('#fff8e1'), spaceBefore=4, spaceAfter=8)

def tbl(data, cw, hdr='#004d40', alt='#e0f2f1'):
...
story.append(Paragraph("Bypass Blue+Yellow Phases — Analysis Report", title_s))
story.append(HRFlowable(width=CW, thickness=2, color=colors.HexColor('#00838f')))
story.append(Spacer(1, 0.5*cm))

meta = [
    ["Scenario:",       "bypass_blue_yellow  (L2 Yellow + L3 Blue phases bypassed)"],
    ...
    ["Observation:",    f"I_L2 reads near-zero in {i2_zero_pct:.1f}% of samples. I_L3 reads near-zero in {i3_zero_pct:.1f}% of samples. I_L1 carries load throughout."],
]
mt = Table(meta, colWidths=[3.8*cm, CW-3.8*cm])
mt.setStyle(TableStyle([
    ('FONTSIZE',       (0,0),(-1,-1), 10),
    ('FONTNAME',       (0,0),(0,-1),  'Helvetica-Bold'),
    ('TEXTCOLOR',      (0,0),(0,-1),  colors.HexColor(DARK_TL)),
    ('ROWBACKGROUNDS', (0,0),(-1,-1), [colors.HexColor('#e0f2f1'), colors.white]),
...
story.append(Paragraph(
    ...
    f"for the <b>bypass_blue_yellow scenario</b>. "
    f"I_L2 (Yellow) reads near-zero in <b>{i2_zero_pct:.1f}%</b> of samples (mean: {i2_mean:.4f}A vs normal {n_i2:.3f}A). "
    f"I_L3 (Blue) reads near-zero in <b>{i3_zero_pct:.1f}%</b> of samples (mean: {i3_mean:.4f}A vs normal {n_i3:.3f}A). "
    f"I_L1 (Red) averages <b>{i1_mean:.3f}A</b> (normal baseline: {n_i1:.3f}A). "
    f"Power factor averages <b>{pf_mean:.4f}</b> (normal: {dfn['PF_total'].mean():.3f}). "
    f"Energy rate: <b>{e_rate:.4f} kWh/h</b> vs normal <b>{n_e_rate:.4f} kWh/h</b>.", info_s))
story.append(PageBreak())

# 1. Data Overview
story.append(Paragraph("1. Data Overview", h1_s))
story.append(HRFlowable(width=CW, thickness=0.5, color=colors.HexColor('#00838f')))
...
story.append(Spacer(1, 0.3*cm))
story.append(Paragraph(
    f"All three phase currents during bypass_blue_yellow are compared against the normal baseline. "
    f"I_L2 (Yellow) reads near-zero in {i2_zero_pct:.1f}% of samples (mean: {i2_mean:.4f}A). "
    f"I_L3 (Blue) reads near-zero in {i3_zero_pct:.1f}% of samples (mean: {i3_mean:.4f}A). "
    f"I_L1 (Red) averages {i1_mean:.3f}A (normal baseline: {n_i1:.3f}A).", body_s))
story.append(Image(os.path.join(CHART_DIR, '01_all_currents.png'), width=CW, height=9*cm))
story.append(Paragraph("Figure 1: All phase currents — normal baseline (top) vs bypass_blue_yellow (bottom).", cap_s))

story.append(Paragraph("2.1  L2 (Yellow)", h2_s))
story.append(Image(os.path.join(CHART_DIR, '02_current_L2_compare.png'), width=CW, height=7.5*cm))
story.append(Paragraph(f"Figure 2: I_L2 — normal baseline (top) vs bypass_blue_yellow (bottom).", cap_s))
story.append(Paragraph(
    f"I_L2 mean: {i2_mean:.4f}A (normal baseline: {n_i2:.3f}A). "
    f"{i2_zero_pct:.1f}% of readings are below the 0.05A threshold.", warn_s))

story.append(PageBreak())
story.append(Paragraph("2.2  L3 (Blue)", h2_s))
story.append(Image(os.path.join(CHART_DIR, '03_current_L3_compare.png'), width=CW, height=7.5*cm))
story.append(Paragraph(f"Figure 3: I_L3 — normal baseline (top) vs bypass_blue_yellow (bottom).", cap_s))
story.append(Paragraph(
    f"I_L3 mean: {i3_mean:.4f}A (normal baseline: {n_i3:.3f}A). "
    f"{i3_zero_pct:.1f}% of readings are below the 0.05A threshold.", warn_s))
...
story.append(Paragraph(
    ...
    f"no sustained voltage collapse was observed on L2 or L3. "
    f"V_L1 mean: {v1_mean:.1f}V | V_L2 mean: {v2_mean:.1f}V | V_L3 mean: {v3_mean:.1f}V.", body_s))
story.append(Image(os.path.join(CHART_DIR, '04_voltage_compare.png'), width=CW, height=8*cm))
story.append(Paragraph("Figure 4: Phase voltages — normal (top) vs bypass_blue_yellow (bottom).", cap_s))
...
story.append(Paragraph(
    ...
    f"Power factor averages <b>{pf_mean:.4f}</b> (normal: {dfn['PF_total'].mean():.3f}).", body_s))
story.append(Image(os.path.join(CHART_DIR, '05_power_compare.png'), width=CW, height=7.5*cm))
story.append(Paragraph("Figure 5: Total active power — normal (top) vs bypass_blue_yellow (bottom).", cap_s))
story.append(Image(os.path.join(CHART_DIR, '06_pf_compare.png'), width=CW, height=6.5*cm))
story.append(Paragraph("Figure 6: Power factor — normal baseline (top) vs bypass_blue_yellow (bottom).", cap_s))
...
story.append(Paragraph("Figure 7: Cumulative energy — normal vs bypass_blue_yellow (normalised to 0 at start).", cap_s))
...
story.append(Spacer(1, 0.3*cm))
story.append(Paragraph(
    "Comparison of key features between normal baseline and bypass_blue_yellow.", body_s))

sig_data = [
    ['Feature', 'Normal', 'Bypass BY', 'Change'],
    ['I_L2 mean (A)',       f'{n_i2:.3f}', f'{i2_mean:.4f}', f'{(i2_mean/max(n_i2,0.001)-1)*100:+.0f}%'],
    ['I_L2 near-zero (%)',  '~20%', f'{i2_zero_pct:.1f}%',   f'+{i2_zero_pct-20:.0f}%'],
    ['I_L3 mean (A)',       f'{n_i3:.3f}', f'{i3_mean:.4f}', f'{(i3_mean/max(n_i3,0.001)-1)*100:+.0f}%'],
    ['I_L3 near-zero (%)',  '~20%', f'{i3_zero_pct:.1f}%',   f'+{i3_zero_pct-20:.0f}%'],
    ['I_L1 mean (A)',       f'{n_i1:.3f}', f'{i1_mean:.3f}', f'{(i1_mean/max(n_i1,0.001)-1)*100:+.0f}%'],
    ['PF total mean',       f'{dfn["PF_total"].mean():.3f}', f'{pf_mean:.4f}', f'{pf_mean-dfn["PF_total"].mean():+.4f}'],
    ['Energy rate (kWh/h)', f'{n_e_rate:.4f}', f'{e_rate:.4f}', f'{(e_rate/n_e_rate-1)*100:+.0f}%'],
    ['I imbalance (std)',   f'{dfn_imb.mean():.3f}A', f'{df_imb.mean():.3f}A', f'+{df_imb.mean()-dfn_imb.mean():.3f}A'],
]
st = Table(sig_data, colWidths=[5*cm, 3*cm, 3*cm, 4.2*cm])
st.setStyle(TableStyle([
    ('BACKGROUND',     (0,0),(-1,0),  colors.HexColor(DARK_TL)),
    ('TEXTCOLOR',      (0,0),(-1,0),  colors.white),
    ('FONTNAME',       (0,0),(-1,0),  'Helvetica-Bold'),
    ('FONTSIZE',       (0,0),(-1,-1), 8.5),
    ('ALIGN',          (1,0),(-1,-1), 'CENTER'),
...
story.append(Spacer(1, 0.5*cm))
story.append(Paragraph(
    f"<b>Summary:</b> In the bypass_blue_yellow scenario over {duration_h:.1f} hours, "
    f"I_L2 reads near-zero in {i2_zero_pct:.1f}% of samples and I_L3 in {i3_zero_pct:.1f}% of samples, "
    f"while I_L1 averages {i1_mean:.3f}A. "
    f"Power factor averages {pf_mean:.4f} compared to {dfn['PF_total'].mean():.3f} in the normal baseline. "
    f"Current imbalance averages {df_imb.mean():.3f}A vs {dfn_imb.mean():.3f}A in the baseline. "
    f"This dataset (label=1) covers a dual-phase near-zero current scenario under varying load conditions.", info_s))

story.append(Spacer(1, 1*cm))
story.append(HRFlowable(width=CW, thickness=1, color=colors.HexColor('#00838f')))
story.append(Paragraph(
    f"Generated by NFE data pipeline  |  {datetime.now().strftime('%d %B %Y, %H:%M')}  |  "
    f"Dataset: bypass_blue_yellow.csv ({len(df):,} rows)  |  Meter: CHINT DTSU666  |  Kampala, Uganda", cap_s))
