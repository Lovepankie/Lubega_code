#!/usr/bin/env python3
"""
Generate charts + PDF analysis report for bypass_red scenario.
...
BYPASS_PATH = os.path.join(BASE_DIR, 'data', 'bypass_red.csv')
NORMAL_PATH = os.path.join(BASE_DIR, 'data', 'normal_final.csv')
CHART_DIR   = os.path.join(BASE_DIR, 'docs', 'bypass_red_charts')
OUTPUT_PDF  = os.path.join(BASE_DIR, 'docs', 'bypass_red_report.pdf')
...
axes[0].plot(dfn['timestamp'], dfn['V_L1'], color='#43a047', linewidth=0.6, alpha=0.8, label='Normal L1')
axes[0].axhline(180, color='red', linestyle='--', linewidth=0.8, label='180V threshold')
axes[0].set_title('Voltage L1 — Normal Baseline', fontsize=11, fontweight='bold')
...
axes[1].plot(df['timestamp'], df['V_L1'], color='#e53935', linewidth=0.6, alpha=0.8, label='Bypass Red L1')
axes[1].axhline(180, color='red', linestyle='--', linewidth=0.8)
axes[1].set_title('Voltage L1 — Bypass Red (anomaly: drops to 0V)', fontsize=11, fontweight='bold', color='#c62828')
...
# 2. Current L1 comparison
fig, axes = plt.subplots(2, 1, figsize=(14, 7), sharex=False)
axes[0].plot(dfn['timestamp'], dfn['I_L1'], color='#43a047', **STYLE, label='Normal I_L1')
axes[0].set_title('Current L1 — Normal Baseline', fontsize=11, fontweight='bold')
axes[0].set_ylabel('A'); axes[0].legend(fontsize=8); axes[0].grid(alpha=0.3)
axes[0].xaxis.set_major_formatter(mdates.DateFormatter(FMT))

axes[1].plot(df['timestamp'], df['I_L1'], color='#e53935', **STYLE, label='Bypass Red I_L1')
axes[1].set_title(f'Current L1 — Bypass Red ({i1_zero_pct:.1f}% readings near zero)', fontsize=11, fontweight='bold', color='#c62828')
...
ax.plot(df['timestamp'], df['I_L1'], color='#e53935', **STYLE, label='I L1 (bypassed)')
ax.plot(df['timestamp'], df['I_L2'], color='#1e88e5', **STYLE, label='I L2')
ax.plot(df['timestamp'], df['I_L3'], color='#43a047', **STYLE, label='I L3')
ax.set_title('All Phase Currents — Bypass Red Scenario', fontsize=13, fontweight='bold')
...
# 4. Power comparison
fig, axes = plt.subplots(2, 1, figsize=(14, 7), sharex=False)
axes[0].plot(dfn['timestamp'], dfn['P_total'], color='#43a047', linewidth=0.6, alpha=0.8)
axes[0].fill_between(dfn['timestamp'], dfn['P_total'], alpha=0.15, color='#43a047')
axes[0].set_title('Total Power — Normal Baseline', fontsize=11, fontweight='bold')
axes[0].set_ylabel('kW'); axes[0].grid(alpha=0.3)
axes[0].xaxis.set_major_formatter(mdates.DateFormatter(FMT))

axes[1].plot(df['timestamp'], df['P_total'], color='#e53935', linewidth=0.6, alpha=0.8)
axes[1].fill_between(df['timestamp'], df['P_total'], alpha=0.15, color='#e53935')
axes[1].set_title('Total Power — Bypass Red', fontsize=11, fontweight='bold', color='#c62828')
...
axes[0].plot(dfn['timestamp'], dfn['PF_total'], color='#43a047', linewidth=0.6, alpha=0.8)
...
axes[1].plot(df['timestamp'], df['PF_total'], color='#e53935', linewidth=0.6, alpha=0.8)
axes[1].set_title('Power Factor — Bypass Red (mean %.3f)' % pf_mean, fontsize=11, fontweight='bold', color='#c62828')
...
ax.plot(range(len(dfn_norm)), dfn_norm.values, color='#43a047', linewidth=1, label='Normal (baseline)')
ax.plot(range(len(df_norm)),  df_norm.values,  color='#e53935', linewidth=1, label='Bypass Red')
...
# 7. Current histogram comparison
fig, axes = plt.subplots(1, 2, figsize=(13, 4))
axes[0].hist(dfn['I_L1'], bins=50, color='#43a047', alpha=0.8, edgecolor='white')
axes[0].set_title('I_L1 Distribution — Normal', fontsize=11, fontweight='bold')
axes[0].set_xlabel('A'); axes[0].set_ylabel('Count'); axes[0].grid(alpha=0.3)

axes[1].hist(df['I_L1'], bins=50, color='#e53935', alpha=0.8, edgecolor='white')
axes[1].set_title('I_L1 Distribution — Bypass Red', fontsize=11, fontweight='bold', color='#c62828')
...
# 8. Hourly average current L1
fig, ax = plt.subplots(figsize=(13, 4))
h_normal = dfn.groupby('hour')['I_L1'].mean()
h_bypass = df.groupby('hour')['I_L1'].mean()
x = range(24)
w = 0.38
ax.bar([i - w/2 for i in x], [h_normal.get(i, 0) for i in x], width=w, color='#43a047', alpha=0.85, label='Normal')
ax.bar([i + w/2 for i in x], [h_bypass.get(i, 0) for i in x], width=w, color='#e53935', alpha=0.85, label='Bypass Red')
ax.set_title('Hourly Average Current L1 — Normal vs Bypass Red', fontsize=13, fontweight='bold')
ax.set_xlabel('Hour of Day'); ax.set_ylabel('Avg I_L1 (A)'); ax.legend(); ax.grid(axis='y', alpha=0.3)
ax.set_xticks(range(24))
plt.tight_layout()
plt.savefig(os.path.join(CHART_DIR, '08_hourly_L1_compare.png'), dpi=150); plt.close()
print("  [8/8] 08_hourly_L1_compare.png")
...
h1_s = ParagraphStyle('H1', parent=styles['Heading1'], fontSize=15, spaceBefore=14,
                spaceAfter=6, textColor=colors.HexColor('#b71c1c'))
...
story.append(Paragraph("Bypass Red Phase — Analysis Report", title_s))
story.append(Spacer(1, 0.2*cm))
story.append(HRFlowable(width=CONTENT_W, thickness=2, color=colors.HexColor('#b71c1c')))
story.append(Spacer(1, 0.5*cm))

meta = [
    ["Scenario:",       "bypass_red  (Phase A / L1 current transformer shunted)"],
...
mt.setStyle(TableStyle([
    ...
    ('TEXTCOLOR',   (0,0),(0,-1), colors.HexColor('#b71c1c')),
...
exec_text = (
    ...
    f"under the <b>bypass_red scenario</b> — Phase A (L1) current transformer shunted with a parallel wire. "
    f"Key anomaly signatures identified: "
    f"Current L1 drops to near-zero in <b>{i1_zero_pct:.1f}%</b> of readings; "
    f"Voltage L1 collapses to 0V in <b>{v_zero:.1f}%</b> of readings; "
...
story.append(HRFlowable(width=CONTENT_W, thickness=0.5, color=colors.HexColor('#b71c1c')))
story.append(Spacer(1, 0.3*cm))
...
for col, lbl in zip(cols, lbls):
    s = df[col].describe()
    diff = s['mean'] - n_means[col]
    nm = f"{n_means[col]:.3f}"
    stat_data.append([lbl, f"{s['mean']:.3f}", f"{s['std']:.3f}",
                      f"{s['min']:.3f}", f"{s['max']:.3f}", nm])
...
story.append(Paragraph(
    ...
    f"During the bypass_red scenario, Phase L1 voltage collapses to <b>0V in {v_zero:.1f}% of readings</b> "
...
story.append(img)
story.append(Paragraph("Figure 1: Voltage L1 — normal baseline (top) vs bypass red (bottom). "
    "Note the frequent collapses to 0V in the bypass scenario.", caption_s))
story.append(Paragraph(
    f"<b>Anomaly signature:</b> V_L1 drops to 0V in {v_zero:.1f}% of samples. "
    f"Normal baseline never drops below 118V. This is the strongest single-feature indicator for bypass_red.", alert_s))
...
story.append(Spacer(1, 0.3*cm))
story.append(Paragraph(
    f"Current L1 (the bypassed phase) averages <b>{i1_mean:.3f}A</b> and is near-zero "
    f"({i1_zero_pct:.1f}% of readings below 0.05A) — confirming that most current is flowing "
    f"through the bypass wire rather than the meter's CT. "
    f"In contrast, the normal baseline shows I_L1 mean of <b>{n_i1:.3f}A</b>. "
    f"Phase L2 and L3 are unaffected and continue showing normal current levels.", body_s))
img2 = Image(os.path.join(CHART_DIR, '02_current_L1_compare.png'), width=CONTENT_W, height=8*cm)
story.append(img2)
story.append(Paragraph("Figure 2: Current L1 — normal (top) vs bypass red (bottom). "
    f"The bypassed phase reads near-zero in {i1_zero_pct:.1f}% of samples.", caption_s))
...
story.append(img3)
story.append(Paragraph("Figure 3: All three phase currents during bypass_red. "
    "I_L1 (red) is suppressed while L2 and L3 behave normally.", caption_s))
story.append(Paragraph(
    f"<b>Anomaly signature:</b> I_L1 mean drops from {n_i1:.3f}A (normal) to {i1_mean:.3f}A (bypass). "
    f"A {(1-i1_mean/max(n_i1,0.001))*100:.0f}% reduction in reported current on the targeted phase.", alert_s))
...
story.append(img4)
story.append(Paragraph("Figure 4: Total active power — normal (top) vs bypass red (bottom).", caption_s))
...
story.append(img5)
story.append(Paragraph("Figure 5: Power factor — normal (top) vs bypass red (bottom). "
...
story.append(img6)
story.append(Paragraph("Figure 6: Cumulative energy gained — normal vs bypass red (both normalised to 0 at start). "
...
story.append(img7)
story.append(Paragraph("Figure 7: I_L1 histogram — normal (left) vs bypass red (right). "
    f"The bypass distribution is heavily concentrated at 0A ({i1_zero_pct:.1f}% of samples).", caption_s))

img8 = Image(os.path.join(CHART_DIR, '08_hourly_L1_compare.png'), width=CONTENT_W, height=5.5*cm)
story.append(img8)
story.append(Paragraph("Figure 8: Hourly average I_L1 — normal vs bypass red by hour of day. "
...
story.append(Spacer(1, 0.3*cm))
story.append(Paragraph(
    "The table below quantifies each anomaly feature for the bypass_red scenario "
    "against the normal baseline, along with the expected detection method.", body_s))

sig_data = [
    ['Feature', 'Normal', 'Bypass Red', 'Change', 'Model'],
    ['V_L1 mean (V)',
     f'{dfn["V_L1"].mean():.1f}', f'{v_mean:.1f}',
     f'{v_mean - dfn["V_L1"].mean():+.1f}V', 'IF / SVM'],
    ['V_L1 zero-collapse (%)',
     '0.0%', f'{v_zero:.1f}%', f'+{v_zero:.1f}%', 'IF / LSTM'],
    ['I_L1 mean (A)',
     f'{n_i1:.3f}', f'{i1_mean:.3f}',
     f'{(i1_mean/max(n_i1,0.001)-1)*100:+.0f}%', 'IF / SVM / LSTM'],
    ['I_L1 near-zero (%)',
     '~5%', f'{i1_zero_pct:.1f}%', f'+{i1_zero_pct-5:.0f}%', 'IF / LSTM'],
    ['PF total mean',
    ...
    ['Current imbalance',
     'Low', 'HIGH (L1 suppressed)', 'Significant', 'IF / SVM'],
]
...
story.append(Paragraph(
    f"<b>Conclusion:</b> The bypass_red scenario produces 4 strong, consistent anomaly signatures: "
    f"voltage collapse, current suppression, PF shift, and reduced energy rate — all on the targeted Phase L1. "
...
story.append(HRFlowable(width=CONTENT_W, thickness=1, color=colors.HexColor('#b71c1c')))
story.append(Spacer(1, 0.2*cm))
story.append(Paragraph(
    f"Generated by NFE data pipeline  |  {datetime.now().strftime('%d %B %Y, %H:%M')}  |  "
    f"Dataset: bypass_red.csv ({len(df):,} rows)  |  Meter: CHINT DTSU666  |  Kampala, Uganda",
    caption_s))
